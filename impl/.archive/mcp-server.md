...
- [x] Handle process signals for clean shutdown
  - **File:** `src/index.ts`
  - **Complexity:** low
  - **Dependencies:** P1.1
  - **Details:**
...
- [x] `bun run dev` starts the MCP server
- [x] Server responds to `tools/list` request
- [x] Server responds to `tools/call` for `list_recordings`
- [x] Server responds to `tools/call` for `get_recording`
- [x] Errors are returned in MCP-compliant format
- [x] Server shuts down cleanly on SIGINT/SIGTERM
